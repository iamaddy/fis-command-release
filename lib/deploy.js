...
function deliver(output, md5, release, content, file, callback){
    ...
    // 被pack过的资源不再产出
    var unreleaselist = fis.config.get('unreleaselist');
    ...
    if(md5 == 0 || !file.useHash){
        ...
        // 不产出被合并后的文件（hash） fix .lc
        if(unreleaselist && unreleaselist.indexOf(release) > -1){
            target = fis.util(output, release).replace(fis.config.get('project.md5Postfix'), "");
        }else{
...
function deploy(dest, file, path, callback){
    if(file.release){
        ...
        fis.log.warning('deploy.js error 文件不存在，unreleasable file [' + file.realpath + ']');
    }
}
...
function doTask(){
    if(tasks.length){
        if(running < exports.MAX_TASK_SIZE){
            ...
            deploy(task.dest, task.file, task.path, function(){
                running--;
                setTimeout(doTask, 0);
            });
        } else {
...
exports = module.exports = function(opt, collection, path, isPkg){
    var dependency = [];
    ...
    fis.util.map(collection, function(subpath, file){
        if(path){
            setDependencies(dependency, path, collection);
        }
    });
    fis.util.map(collection, function(subpath, file){
        dests.forEach(function(d){
            if(
                ...
                if(path){
                    ...
                    // 依赖文件
                    // 雪碧图
                    if(file.subpath.indexOf(path) > -1 || 
                        file.origin === path ||
                        (fis.cli.commander.args[0].watch ? false : dependency.indexOf(file.subpath) > -1) || 
                        (file.useSprite && file.rExt === '.png')){
                       tasks.push({dest: d, file: file, path: path});
                    }
                }else{
    ...
    doTask();
    function setDependencies(dependency, path, ret) {
        try{
            var depend = ret[path] && ret[path].dependency;
            if (depend)
                depend.forEach(function(path) {
                    !~dependency.indexOf(path) && dependency.push(path);
                    setDependencies(dependency, path, ret)
                })
        }catch(ex){
            console.log(ex);
        }
